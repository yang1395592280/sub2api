...
type AccountGroup struct {
	...
	// Whether OpenAI channel-price grouping must preserve this membership
	PriceGroupingLocked bool `json:"price_grouping_locked,omitempty"`
	...
	for i := range columns {
		...
		case accountgroup.FieldPriceGroupingLocked:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field price_grouping_locked", values[i])
			} else if value.Valid {
				_m.PriceGroupingLocked = value.Bool
			}
		case accountgroup.FieldCreatedAt:
...
func (_m *AccountGroup) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("price_grouping_locked=")
	builder.WriteString(fmt.Sprintf("%v", _m.PriceGroupingLocked))
	builder.WriteString(", ")
