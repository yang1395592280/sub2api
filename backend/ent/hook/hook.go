...
// The WindsurfAccountFunc type is an adapter to allow the use of ordinary
// function as WindsurfAccount mutator.
type WindsurfAccountFunc func(context.Context, *ent.WindsurfAccountMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WindsurfAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.WindsurfAccountMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WindsurfAccountMutation", m)
}
