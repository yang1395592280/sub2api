...
// The OpenAISchedulerHealthStateFunc type is an adapter to allow the use of ordinary
// function as OpenAISchedulerHealthState mutator.
type OpenAISchedulerHealthStateFunc func(context.Context, *ent.OpenAISchedulerHealthStateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OpenAISchedulerHealthStateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OpenAISchedulerHealthStateMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OpenAISchedulerHealthStateMutation", m)
}
