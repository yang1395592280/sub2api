...
func (h *SizeBetHandler) GetHistory(c *gin.Context) {
	...
	response.PaginatedWithResult(c, toSizeBetUserHistoryResponses(items), toResponsePagination(paginationResult))
}

func (h *SizeBetHandler) ListRecentRounds(c *gin.Context) {
	_, pageSize := response.ParsePagination(c)
	items, err := h.service.ListRecentRounds(c.Request.Context(), pageSize)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	response.PaginatedWithResult(c, toSizeBetRoundSummaryResponses(items), toResponsePagination(paginationResult))
}
...
func (h *SizeBetHandler) ListStatsUsers(c *gin.Context) {
	...
	response.PaginatedWithResult(c, toSizeBetStatsUserResponses(items), toResponsePagination(paginationResult))
}
