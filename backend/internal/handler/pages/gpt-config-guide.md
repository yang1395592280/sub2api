...
本文中的中转站地址已经配置为：`https://www.loomex.lol`
...
确认后点击右下角“导入”。完成后，CC Switch 的 Codex 列表中会出现 `tokenbook` 卡片。

### 3. 将 Tokenbook 设为当前配置

在 `tokenbook` 卡片右侧点击“使用”，让 CC Switch 把它写入当前 Codex 配置。按钮由“使用”变成“使用中”即表示完成。

### 4. 测试连接并重新打开 Codex

点击卡片右侧的连接测试图标。测试通过后，完全退出 Codex，再重新打开。

顶部显示绿色“tokenbook 连通正常”和延迟时间，即表示连接已完成。卡片显示“使用中”、连接测试显示“连通正常”后，重新打开 Codex 即可开始新对话。
