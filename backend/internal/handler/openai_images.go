...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		timing.BeginRouting()
		effectiveAPIKey, effectiveRoutingModel, selection, scheduleDecision, err := h.gatewayService.SelectEffectiveOpenAIAccountWithSchedulerForImagesAndModelResolver(
			requestCtx,
			apiKey,
			sessionHash,
			routingModel,
		...
		if autoGroupMode {
			if !service.GroupAllowsImageGeneration(apiKeyForRequest.Group) {
			...
			channelMapping, _ = h.gatewayService.ResolveChannelMappingAndRestrict(c.Request.Context(), apiKeyForRequest.GroupID, routingModel)
			if strings.TrimSpace(effectiveRoutingModel) != "" {
				channelMapping.MappedModel = effectiveRoutingModel
			}
		...
		accountReleaseFunc, slotResult := h.acquireResponsesAccountSlot(c, apiKeyForRequest.GroupID, sessionHash, selection, parsed.Stream, &streamStarted, reqLog)
