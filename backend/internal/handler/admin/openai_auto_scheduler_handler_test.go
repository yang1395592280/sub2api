...
func TestOpenAIAutoSchedulerHandler_ListScoresReturnsPaginatedRows(t *testing.T) {
	schedulerSvc := &fakeOpenAIAutoSchedulerService{
		total: 2,
		scores: []service.OpenAIAutoSchedulerScoreState{
			{AccountID: 101, GroupID: 20, Model: "gpt-5", FinalScore: 8750, State: service.OpenAIAutoSchedulerStateRunning},
			{AccountID: 102, GroupID: 20, Model: "gpt-5", FinalScore: 6400, State: service.OpenAIAutoSchedulerStateOpen},
		},
	}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/openai-auto-scheduler/scores?page=2&page_size=1&group_id=20&model=gpt-5", nil)
	router.ServeHTTP(rec, req)
