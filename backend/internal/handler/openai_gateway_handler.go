...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	// 被动声明 image_gen namespace 不代表本次请求一定生图，不能据此拒绝请求或占用生图槽位。
	...
	for {
		...
		accountReleaseFunc, slotResult := h.acquireResponsesAccountSlot(c, apiKeyForRequest.GroupID, sessionHash, selection, reqStream, &streamStarted, reqLog)
		...
		h.recordCyberPolicyIfMarked(c, apiKeyForRequest, account, subscription, reqModel, err != nil, cyberBlockKeyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(body))
		...
		submitResponsesUsage := func(res *service.OpenAIForwardResult) {
			...
			quotaPlatform := service.QuotaPlatform(c.Request.Context(), apiKeyForRequest)
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if service.IsOpenAIFirstOutputTimeoutError(failoverErr) {
						service.RequireOpenAIAutoCheapestQualifiedFailover(c.Request.Context())
					}
					...
					if failoverErr.RetryableOnSameAccount {
						...
						if sameAccountRetryAllowed(failoverErr, sameAccountRetryCount[account.ID], retryLimit) {
							...
							if !waitOpenAIRetryDelay(c.Request.Context(), timing, retryDelay) {
								return
							}
							continue
						}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	if !autoGroupModeMsg && !allowOpenAICompatibleMessagesDispatch(c, apiKey) {
	...
	for {
		...
		h.recordCyberPolicyIfMarked(c, apiKeyForRequest, account, subscription, reqModel, err != nil, cyberBlockKeyMsg, clientRequestedUsageFields(c, channelMappingMsg, reqModel, ""), service.HashUsageRequestPayload(body))
		forwardDurationMs := time.Since(forwardStart).Milliseconds()
		upstreamLatencyMs, _ := getContextInt64(c, service.OpsUpstreamLatencyMsKey)
		responseLatencyMs := forwardDurationMs
		if upstreamLatencyMs > 0 && forwardDurationMs > upstreamLatencyMs {
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	imageIntentWS := service.IsExplicitImageGenerationIntent("/v1/responses", reqModel, firstMessage)
	...
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError) bool {
		...
		if service.IsOpenAIFirstOutputTimeoutError(failoverErr) {
			service.RequireOpenAIAutoCheapestQualifiedFailover(ctx)
		}
		if openAIFirstOutputFailoverExhausted(failoverErr, &firstOutputTimeoutSwitchCount) {
	...
	if imageIntentWS && requestPlatform == service.PlatformOpenAI {
		requiredCapability = service.OpenAIEndpointCapabilityResponses
	}
	var wsSelectionModelResolver func(*service.APIKey, string) string
	if autoGroupModeWS {
		// 自动分组必须按候选分组解析渠道映射；普通分组仍按客户端模型筛选账号，
		// 允许渠道目标仅作为上游模型而不出现在账号映射键中。
		wsSelectionModelResolver = h.openAIChannelMappedModelResolver(ctx)
	}
	...
	for {
		...
		if err := h.gatewayService.BindStickySessionAfterProfitAdmission(ctx, apiKeyForRequest.GroupID, sessionHash, account.ID); err != nil {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				if snapshot := turnChannelMapping.Load(); snapshot != nil && snapshot.turn == turn {
					...
					turnMapping, _ = h.gatewayService.ResolveChannelMappingAndRestrict(ctx, apiKeyForRequest.GroupID, turnRequestedModel)
				}
				if turnUpstreamModel == "" {
					turnUpstreamModel = turnRequestedModel
				}
				turnUsageFields := turnMapping.ToUsageFields(turnRequestedModel, turnUpstreamModel)
				h.recordCyberPolicyIfMarked(c, apiKeyForRequest, account, subscription, turnRequestedModel, turnErr != nil, cyberBlockKey, turnUsageFields, requestPayloadHash)
				if service.GetOpsCyberPolicy(c) != nil {
...
func openAIFirstOutputFailoverExhausted(failoverErr *service.UpstreamFailoverError, switchCount *int) bool {
	if failoverErr == nil || (!service.IsOpenAIFirstOutputTimeoutError(failoverErr) && !failoverErr.SafeToFailoverAfterWrite) || switchCount == nil {
		return false
	}
