...
func TestUsageLogRepositoryGetUserSpendingRankingUsesRequestedSort(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "actual_cost", "requests", "tokens", "total", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(2), "beta@example.com", "beta", 12.5, int64(9), int64(900), int64(23), 40.0, int64(30), int64(2600))
...
func TestPrepareUsageLogInsert_PersistsChannelPriceSnapshot(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, &price, prepared.args[64])
	require.Equal(t, sql.NullString{String: source, Valid: true}, prepared.args[65])
	require.Equal(t, sql.NullTime{Time: refreshedAt, Valid: true}, prepared.args[66])
}
...
func TestScanUsageLogSchedulerTimingAndChannelPriceSnapshot(t *testing.T) {
	...
	rowValues := []any{
		...
		nil,                               // requested_reasoning_effort
...
func TestUsageLogRepositoryGetUserSpendingRanking(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "actual_cost", "requests", "tokens", "total", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(2), "beta@example.com", "beta", 12.5, int64(9), int64(900), int64(23), 40.0, int64(30), int64(2600)).
		AddRow(int64(1), "alpha@example.com", "alpha", 12.5, int64(8), int64(800), int64(23), 40.0, int64(30), int64(2600)).
