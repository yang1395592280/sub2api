...
func TestBusinessAnalyticsRepository_GetOverviewIncludingTodayUsesRateMultiplierCost(t *testing.T) {
	todayStart := time.Now().UTC().Truncate(24 * time.Hour)
	query, _ := buildBusinessAggregateQuery(service.BusinessAnalyticsFilter{
	...
	require.Contains(t, query, "COALESCE(SUM(COALESCE(COALESCE(ul.actual_cost, 0) / NULLIF(ul.rate_multiplier, 0), COALESCE(ul.account_stats_cost, ul.total_cost, 0)) * COALESCE(ul.account_rate_multiplier, 1)), 0)")
	require.NotContains(t, query, "COALESCE(ul.channel_price_snapshot, COALESCE(ul.account_rate_multiplier, 1))")
}
...
func TestBusinessAnalyticsRepository_GetGroupsIncludingTodayAliasesPreviousPeriod(t *testing.T) {
	...
	mock.ExpectQuery(containsAllRegexp(
		...
		"COALESCE(SUM(COALESCE(COALESCE(ul.actual_cost, 0) / NULLIF(ul.rate_multiplier, 0), COALESCE(ul.account_stats_cost, ul.total_cost, 0)) * COALESCE(ul.account_rate_multiplier, 1)), 0) AS channel_cost",
		"COALESCE(SUM(ul.actual_cost), 0) - COALESCE(SUM(COALESCE(COALESCE(ul.actual_cost, 0) / NULLIF(ul.rate_multiplier, 0), COALESCE(ul.account_stats_cost, ul.total_cost, 0)) * COALESCE(ul.account_rate_multiplier, 1)), 0) AS gross_profit",
...
func TestBusinessAnalyticsRepository_GetRecordsUsesRateMultiplierCost(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	mock.ExpectQuery(containsAllRegexp(
		"COALESCE(COALESCE(COALESCE(ul.actual_cost, 0) / NULLIF(ul.rate_multiplier, 0), COALESCE(ul.account_stats_cost, ul.total_cost, 0)) * COALESCE(ul.account_rate_multiplier, 1), 0) AS channel_cost",
		"COALESCE(ul.actual_cost, 0) - COALESCE(COALESCE(COALESCE(ul.actual_cost, 0) / NULLIF(ul.rate_multiplier, 0), COALESCE(ul.account_stats_cost, ul.total_cost, 0)) * COALESCE(ul.account_rate_multiplier, 1), 0) AS gross_profit",
		"ul.rate_multiplier",
