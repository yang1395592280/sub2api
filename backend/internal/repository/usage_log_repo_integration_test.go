...
func (s *UsageLogRepoSuite) TestDashboardStats_TodayTotalsAndPerformance() {
	...
	mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: userToday.ID, Key: "sk-ul-auto-1", Name: "auto1", GroupSelectMode: service.APIKeyGroupSelectModeOpenAIAutoCheapest})
	mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: userToday.ID, Key: "sk-ul-auto-2", Name: "auto2", GroupSelectMode: service.APIKeyGroupSelectModeOpenAIAutoCheapest})
	mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: userOld.ID, Key: "sk-ul-fixed", Name: "fixed"})
	...
	s.Require().Equal(baseStats.TotalAPIKeys+5, stats.TotalAPIKeys, "TotalAPIKeys mismatch")
	s.Require().Equal(baseStats.ActiveAPIKeys+1, stats.ActiveAPIKeys, "ActiveAPIKeys mismatch")
	s.Require().Equal(baseStats.OpenAIAutoCheapestUsers+1, stats.OpenAIAutoCheapestUsers, "OpenAIAutoCheapestUsers mismatch")
