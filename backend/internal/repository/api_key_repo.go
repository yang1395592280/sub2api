...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetNillableOpenaiAutoGroupMaxRateMultiplier(key.OpenAIAutoGroupMaxRateMultiplier).
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldOpenaiAutoGroupMaxRateMultiplier,
			apikey.FieldName,
			apikey.FieldStatus,
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldOpenaiAutoSchedulerEnabled,
				group.FieldAllowAutoCheapestScheduling,
				...
				group.FieldUpstreamPriceGroupingEnabled,
				group.FieldUpstreamPriceGroupingMin,
				group.FieldUpstreamPriceGroupingMax,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.GroupSelection {
		builder.SetGroupSelectMode(key.NormalizedGroupSelectMode())
		if key.OpenAIAutoGroupMaxRateMultiplier != nil {
			builder.SetOpenaiAutoGroupMaxRateMultiplier(*key.OpenAIAutoGroupMaxRateMultiplier)
		} else {
			builder.ClearOpenaiAutoGroupMaxRateMultiplier()
		}
	}
...
func (r *apiKeyRepository) ClearGroupIDByUserAndGroup(ctx context.Context, userID, groupID int64) (int64, error) {
	client := clientFromContext(ctx, r.client)
	n, err := client.APIKey.Update().
		Where(apikey.UserIDEQ(userID), apikey.GroupIDEQ(groupID), apikey.DeletedAtIsNil()).
		ClearGroupID().
		Save(ctx)
	return int64(n), err
}
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		OpenAIAutoGroupMaxRateMultiplier: m.OpenaiAutoGroupMaxRateMultiplier,
