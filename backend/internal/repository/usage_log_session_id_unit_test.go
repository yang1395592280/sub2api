...
// is immediately before native_compaction_v2; timing fields and created_at follow.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 74, "arg-type table must include session_id and timing fields")
	...
	// session_id is followed by native_compaction_v2, timing fields, and created_at.
...
func TestPrepareUsageLogInsert_RequestedReasoningEffortArgWiring(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, "text", usageLogInsertArgTypes[54], "requested_reasoning_effort must follow reasoning_effort")
	require.Equal(t, "text", usageLogInsertArgTypes[53], "reasoning_effort arg type must stay text")

	forwardedArg, ok := prepared.args[53].(sql.NullString)
	require.True(t, ok)
	require.True(t, forwardedArg.Valid)
	require.Equal(t, forwarded, forwardedArg.String)

	requestedArg, ok := prepared.args[54].(sql.NullString)
	require.True(t, ok)
