...
func buildUsageLogBestEffortInsertQuery(preparedList []usageLogInsertPrepared) (string, []any) {
	...
	args := make([]any, 0, len(preparedList)*(len(usageLogInsertArgTypes)+1))
	argPos := 1
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	e2eFirstToken := nullInt(log.E2EFirstTokenMs)
	routing := nullInt(log.RoutingMs)
	queue := nullInt(log.QueueMs)
	retry := nullInt(log.RetryMs)
