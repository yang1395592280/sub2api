...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		channelPriceRefreshedAt   sql.NullTime
		upstreamRequestID         sql.NullString
		sessionID                 sql.NullString
		nativeCompactionV2        bool
		bodyReadMs                sql.NullInt64
		preprocessMs              sql.NullInt64
		userQueueMs               sql.NullInt64
		createdAt                 time.Time
	)
	...
	if apiKeyGroupSelectMode.Valid {
		log.APIKeyGroupSelectMode = &apiKeyGroupSelectMode.String
	}
	if durationMs.Valid {
	...
	if bodyReadMs.Valid {
		value := int(bodyReadMs.Int64)
		log.BodyReadMs = &value
	}
	if preprocessMs.Valid {
		value := int(preprocessMs.Int64)
		log.PreprocessMs = &value
	}
	if userQueueMs.Valid {
		value := int(userQueueMs.Int64)
		log.UserQueueMs = &value
	}
	if userAgent.Valid {
	...
	if channelPriceSnapshot.Valid {
		log.ChannelPriceSnapshot = &channelPriceSnapshot.Float64
	}
	if channelPriceSource.Valid {
		log.ChannelPriceSource = &channelPriceSource.String
	}
	if channelPriceRefreshedAt.Valid {
		log.ChannelPriceRefreshedAt = &channelPriceRefreshedAt.Time
	}
	if sessionID.Valid {
