...
// GetAccountStabilityStatsBatch 批量聚合账号在指定窗口内的成功请求、失败请求和平均耗时。
func (r *usageLogRepository) GetAccountStabilityStatsBatch(ctx context.Context, accountIDs []int64, startTime, endTime time.Time) (map[int64]*service.AccountStabilityStats, error) {
	result := make(map[int64]*service.AccountStabilityStats, len(accountIDs))
	if len(accountIDs) == 0 {
		return result, nil
	}
	...
	query := `
		...
		SELECT
			...
			COALESCE(successes.success_count, 0) AS success_count,
			COALESCE(errors.error_count, 0) AS error_count,
			successes.avg_duration_ms
		FROM account_ids
		LEFT JOIN successes ON successes.account_id = account_ids.account_id
		LEFT JOIN errors ON errors.account_id = account_ids.account_id
	`
	...
	for rows.Next() {
		var (
			...
			successCount  int64
			errorCount    int64
			avgDurationMs sql.NullInt64
		)
		if err := rows.Scan(&accountID, &successCount, &errorCount, &avgDurationMs); err != nil {
			return nil, err
		}
		stats := &service.AccountStabilityStats{
			SuccessCount: successCount,
			ErrorCount:   errorCount,
		}
		if avgDurationMs.Valid {
			v := int(avgDurationMs.Int64)
	...
	for _, accountID := range accountIDs {
		if _, ok := result[accountID]; !ok {
			result[accountID] = &service.AccountStabilityStats{}
		}
	}
	return result, nil
}
