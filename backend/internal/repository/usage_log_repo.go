...
func appendAPIKeyGroupSelectModeWhereCondition(conditions []string, args []any, mode string, alias string) ([]string, []any) {
	mode = strings.TrimSpace(mode)
	if mode == "" {
		return conditions, args
	}
	usageAlias := "usage_logs"
	if alias != "" {
		usageAlias = alias
	}
	conditions = append(conditions, fmt.Sprintf("EXISTS (SELECT 1 FROM api_keys ak WHERE ak.id = %s.api_key_id AND ak.group_select_mode = $%d)", usageAlias, len(args)+1))
...
func appendAPIKeyGroupSelectModeQueryFilter(query string, args []any, mode string, alias string) (string, []any) {
	conditions, args := appendAPIKeyGroupSelectModeWhereCondition(nil, args, mode, alias)
	if len(conditions) == 0 {
		return query, args
	}
	return query + " AND " + conditions[0], args
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			nullTime(log.ChannelPriceRefreshedAt),
			createdAt,
		},
	}
}
...
func buildUserSpendingRankingOrderClause(sortBy, sortOrder string) string {
	order := strings.ToUpper(usagestats.NormalizeUserSpendingRankingOrder(sortOrder))
	switch usagestats.NormalizeUserSpendingRankingSort(sortBy) {
	case usagestats.RankingSortRequests:
		return fmt.Sprintf("requests %s, actual_cost DESC, user_id ASC", order)
	case usagestats.RankingSortTokens:
		return fmt.Sprintf("tokens %s, actual_cost DESC, user_id ASC", order)
	default:
		return fmt.Sprintf("actual_cost %s, tokens DESC, user_id ASC", order)
	}
}

// GetUserSpendingRanking returns user spending ranking aggregated within the time range.
func (r *usageLogRepository) GetUserSpendingRanking(ctx context.Context, startTime, endTime time.Time, page, pageSize int, sortBy, sortOrder string) (result *UserSpendingRankingResponse, err error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
	...
	orderClause := buildUserSpendingRankingOrderClause(sortBy, sortOrder)
	...
	rows, err := r.sql.QueryContext(ctx, query, startTime, endTime, pageSize, offset)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.UserID, &row.Email, &row.ActualCost, &row.Requests, &row.Tokens, &total, &totalActualCost, &totalRequests, &totalTokens); err != nil {
			return nil, err
		}
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendAPIKeyGroupSelectModeWhereCondition(conditions, args, filters.APIKeyGroupSelectMode, "")
...
func (r *usageLogRepository) GetUsageTrendWithUsageFilters(ctx context.Context, startTime, endTime time.Time, granularity string, filters UsageLogFilters) (results []TrendDataPoint, err error) {
	return r.getUsageTrendWithFilters(ctx, startTime, endTime, granularity, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.APIKeyGroupSelectMode, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
}

func (r *usageLogRepository) getUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, apiKeyGroupSelectMode string, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []TrendDataPoint, err error) {
	if shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, apiKeyGroupSelectMode, model, requestType, stream, billingType, billingMode) {
	...
	query, args = appendAPIKeyGroupSelectModeQueryFilter(query, args, apiKeyGroupSelectMode, "")
...
func shouldUsePreaggregatedTrend(granularity string, userID, apiKeyID, accountID, groupID int64, apiKeyGroupSelectMode string, model string, requestType *int16, stream *bool, billingType *int8, billingMode string) bool {
...
func (r *usageLogRepository) GetModelStatsWithUsageFiltersBySource(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters, source string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.APIKeyGroupSelectMode, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, source, filters.BillingMode)
}

func (r *usageLogRepository) getModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, apiKeyGroupSelectMode string, model string, requestType *int16, stream *bool, billingType *int8, source string, billingMode string) (results []ModelStat, err error) {
...
func (r *usageLogRepository) GetGroupStatsWithUsageFilters(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters) (results []usagestats.GroupStat, err error) {
	return r.getGroupStatsWithFilters(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.APIKeyGroupSelectMode, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
}

func (r *usageLogRepository) getGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, apiKeyGroupSelectMode string, model string, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
	...
	query, args = appendAPIKeyGroupSelectModeQueryFilter(query, args, apiKeyGroupSelectMode, "ul")
	if strings.TrimSpace(model) != "" {
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		channelPriceSnapshot  sql.NullFloat64
		channelPriceSource    sql.NullString
		channelPriceRefreshed sql.NullTime
		createdAt             time.Time
	)
	...
	log := &service.UsageLog{
		...
		ChannelPriceSnapshot:  nullFloat64Ptr(channelPriceSnapshot),
		BillingType:           int8(billingType),
		RequestType:           service.RequestTypeFromInt16(requestTypeRaw),
		ImageCount:            imageCount,
		CacheTTLOverridden:    cacheTTLOverridden,
		CreatedAt:             createdAt,
		APIKeyGroupSelectMode: nullStringPtr(apiKeyGroupSelectMode),
	}
	...
	if channelPriceSource.Valid {
		log.ChannelPriceSource = &channelPriceSource.String
	}
	if channelPriceRefreshed.Valid {
		log.ChannelPriceRefreshedAt = &channelPriceRefreshed.Time
	}
