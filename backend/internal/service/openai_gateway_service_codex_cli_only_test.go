...
func TestShouldFailoverOpenAIUpstreamResponseContextWindow502(t *testing.T) {
	...
	require.False(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusBadGateway, "", body))
	require.True(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusBadGateway, "temporary upstream outage", []byte(`{"error":{"message":"temporary upstream outage"}}`)))
	require.True(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(),
		http.StatusBadGateway,
