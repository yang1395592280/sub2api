...
func TestOpenAIUpstreamAccessStateClassification(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, (&OpenAIGatewayService{}).shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusForbidden, "", body))
...
func TestOpenAIHTTPAccessStateDoesNotTrustBadRequestMessage(t *testing.T) {
	...
	require.False(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusBadRequest, "", body))
	require.False(t, shouldFailoverOpenAIPassthroughResponse(&Account{Type: AccountTypeOAuth}, http.StatusBadRequest, body))
...
func TestOpenAIHTTPAccessStateTrustsStructuredCode(t *testing.T) {
	...
	require.True(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusBadRequest, "", body))
...
func TestOpenAICyberPolicyWrapped5xxNeverFailsOver(t *testing.T) {
	...
	svc := &OpenAIGatewayService{}
	require.False(t, svc.shouldFailoverOpenAIUpstreamResponseForAccount(newOpenAIUpstreamErrorTestAccount(), http.StatusBadGateway, "wrapped upstream failure", body))
