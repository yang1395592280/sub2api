...
func TestOpenAIResponsesTTFTStartsAtSemanticOutput(t *testing.T) {
...
func TestOpenAINativeProgressStartsTTFTBeforeTimeout(t *testing.T) {
	result := runSyntheticVisibleTTFTStream(t, false, 1200*time.Millisecond, 1,
		`{"type":"response.output_text.delta","delta":"test output"}`)
	require.NotNil(t, result.firstTokenMs)
	require.Less(t, *result.firstTokenMs, 1100)
}
