...
const (
	SettingKeySizeBetRoundDurationSeconds  = "size_bet_round_duration_seconds"
	SettingKeySizeBetBetCloseOffsetSeconds = "size_bet_bet_close_offset_seconds"
	SettingKeySizeBetAllowedStakes         = "size_bet_allowed_stakes"
	SettingKeySizeBetCustomStakeMin        = "size_bet_custom_stake_min"
	SettingKeySizeBetCustomStakeMax        = "size_bet_custom_stake_max"
	SettingKeySizeBetProbabilities         = "size_bet_probabilities"
	SettingKeySizeBetOdds                  = "size_bet_odds"
	SettingKeySizeBetRulesMarkdown         = "size_bet_rules_markdown"
)
...
func isValidSizeBetOdds(cfg SizeBetOddsConfig) bool {
	return isIntegralPositiveOdd(cfg.Small) &&
		isIntegralPositiveOdd(cfg.Mid) &&
		isIntegralPositiveOdd(cfg.Big)
}

func isIntegralPositiveOdd(value float64) bool {
	if value <= 0 {
		return false
	}
...
func mustJSON(v any) string {
	...
	if err != nil {
		panic(fmt.Sprintf("marshal default json failed: %v", err))
	}
	return string(raw)
}
