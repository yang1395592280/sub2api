...
// normalizeOpenAICompatiblePlatform keeps the custom scheduler call sites
// compatible with the exported upstream helper.
func normalizeOpenAICompatiblePlatform(platform string) string {
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, openAISelectionFilterStats) {
	...
	for i := range accounts {
		...
		fresh = s.recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx, fresh, accountSourceGroupID, platform, requestedModel, false, requiredCapability)
		if fresh == nil {
			filterStats.exclude("ineligible")
			continue
		}
	...
	if ranked, used := s.rankOpenAIAutoSchedulerCandidates(ctx, groupID, platform, requestedModel, candidates); used {
		...
		return ranked[0], compactBlocked, filterStats
	}
	...
	return candidates[0], compactBlocked, filterStats
}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	rateOrder := openAILegacyUpstreamRateOrder{}
	if !autoSchedulerRanked && preferLowUpstreamRate {
		rateOrder = newOpenAILegacyUpstreamRateOrder(candidates, time.Now(), s.openAIOAuthSchedulingRateMultiplier(ctx))
	}
