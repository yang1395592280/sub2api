...
func TestAPIKeyUpdate_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	autoMode := APIKeyGroupSelectModeOpenAIAutoCheapest
	...
	tests := []struct {
		...
		{
			name: "switch to automatic group selection",
			req:  UpdateAPIKeyRequest{GroupSelectMode: &autoMode},
			want: APIKeyUpdateFields{GroupID: true, GroupSelection: true},
		},
	}
...
func TestAPIKeyUpdate_DeclaresUsageColumnsOnExplicitReset(t *testing.T) {
	reset := true
	svc, repo := newUpdateFieldsAPIKeyService(&APIKey{
		ID: 1, UserID: 7, Key: "sk-test", GroupID: ptrInt64(1), Status: StatusActive, Quota: 100, QuotaUsed: 30, Usage5h: 12,
	})
...
func TestAPIKeyUpdate_DeclaresStatusWhenReactivated(t *testing.T) {
	quota := 500.0
	svc, repo := newUpdateFieldsAPIKeyService(&APIKey{
		ID: 1, UserID: 7, Key: "sk-test", GroupID: ptrInt64(1), Status: StatusAPIKeyQuotaExhausted, Quota: 100, QuotaUsed: 100,
	})
