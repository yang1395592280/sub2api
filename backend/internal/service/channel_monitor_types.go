...
// ChannelMonitorWindowStats 是健康看板按时间窗口聚合后的主模型统计。
type ChannelMonitorWindowStats struct {
	TotalChecks       int
	...
	AvgLatencyMs      *int
	P95LatencyMs      *int
	AvgPingLatencyMs  *int
}

// OpenAIHealthQuery 描述 OpenAI 健康看板聚合查询条件。
type OpenAIHealthQuery struct {
	GroupName string
	Search    string
	Window    string
	Now       time.Time
...
// OpenAIHealthOverview 是 OpenAI 健康看板聚合响应。
type OpenAIHealthOverview struct {
	...
	FailedMonitors         int                `json:"failed_monitors"`
	AverageAvailabilityPct float64            `json:"average_availability_pct"`
	AverageFirstTokenMs    *int               `json:"average_first_token_ms,omitempty"`
	Items                  []OpenAIHealthItem `json:"items"`
}

// OpenAIHealthItem 是健康看板表格中的单行监控统计。
type OpenAIHealthItem struct {
	ID                  int64                    `json:"id"`
	...
	LatestFirstTokenMs  *int                     `json:"latest_first_token_ms,omitempty"`
	...
	AvgFirstTokenMs     *int                     `json:"avg_first_token_ms,omitempty"`
	P95FirstTokenMs     *int                     `json:"p95_first_token_ms,omitempty"`
	AvgPingLatencyMs    *int                     `json:"avg_ping_latency_ms,omitempty"`
	Trend               []OpenAIHealthTrendPoint `json:"trend"`
}

// OpenAIHealthTrendPoint 是健康看板 sparkline 的单点数据。
type OpenAIHealthTrendPoint struct {
	Status        string    `json:"status"`
