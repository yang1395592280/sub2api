...
func (s *sizeBetQueryRepoStub) ListUserHistory(_ context.Context, userID int64, params pagination.PaginationParams) ([]SizeBetUserHistoryItem, *pagination.PaginationResult, error) {
	...
	if result == nil {
		result = &pagination.PaginationResult{Total: int64(len(s.historyItems)), Page: params.Page, PageSize: params.PageSize}
	}
	return append([]SizeBetUserHistoryItem(nil), s.historyItems...), result, nil
}
...
func (s *sizeBetQueryRepoStub) ListAdminRounds(context.Context, pagination.PaginationParams) ([]SizeBetRound, *pagination.PaginationResult, error) {
	return append([]SizeBetRound(nil), s.latestRounds...), &pagination.PaginationResult{Total: int64(len(s.latestRounds)), Page: 1, PageSize: 20, Pages: 1}, nil
}
...
func TestSizeBetServiceGetCurrentRoundViewReturnsCurrentPhaseAndPreviousRound(t *testing.T) {
	...
	svc := NewSizeBetService(repo, &sizeBetRuntimeSettingRepoStub{values: map[string]string{}}, nil)
...
func TestSizeBetRuntimeServiceRunOnceEnsuresCurrentRoundAndSettlesDueRounds(t *testing.T) {
	now := time.Date(2026, 4, 23, 12, 1, 5, 0, time.UTC)
	repo := &sizeBetQueryRepoStub{
		roundsDue: []SizeBetRound{{
			ID:              1001,
			RoundNo:         1001,
		...
		latestRounds: []SizeBetRound{{ResultDirection: SizeBetDirectionSmall}, {ResultDirection: SizeBetDirectionSmall}},
	}
	svc := NewSizeBetService(repo, &sizeBetRuntimeSettingRepoStub{values: map[string]string{}}, nil)
	svc.now = func() time.Time { return now }
	runtime := NewSizeBetRuntimeService(svc, time.Second)

	runtime.runOnce()

	require.GreaterOrEqual(t, repo.createRoundCalls, 1)
...
func TestSizeBetRuntimeServiceStopStopsBackgroundWorker(t *testing.T) {
	...
	runtime := NewSizeBetRuntimeService(svc, 10*time.Millisecond)
