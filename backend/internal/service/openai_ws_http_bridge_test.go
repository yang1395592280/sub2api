...
func TestOpenAIWSHTTPBridgeRelaysSSEFramesAsWebSocketMessages(t *testing.T) {
	...
	account := &Account{ID: 7, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1, Status: StatusActive}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "response.completed", result.WSTerminalEventType)
	require.Len(t, events, 3)
	require.Equal(t, "response.completed", gjson.GetBytes(events[2], "type").String())
}
