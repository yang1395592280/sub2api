...
const (
	...
	RedeemTypeLotteryTicket    = domain.RedeemTypeLotteryTicket
...
const (
	...
	SettingKeyAffiliateTicketCampaignEnabled   = "affiliate_ticket_campaign_enabled"   // 邀请抽奖券活动总开关
	...
	SettingKeyJoinGroupEnabled            = "join_group_enabled"            // 是否展示加入群聊入口
	SettingKeyJoinGroupURL                = "join_group_url"                // 加入群聊跳转链接
	SettingKeyJoinGroupPopupImage         = "join_group_popup_image"        // 加入群聊弹窗图片（data URL）
	...
	// OpenAI 动态计费固定利润（按每美元基础模型成本的倍率单位配置）。
	SettingKeyOpenAIDynamicBillingProfitMarkup = "openai_dynamic_billing_profit_markup"
