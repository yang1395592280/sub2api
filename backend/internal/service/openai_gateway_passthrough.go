...
// Structural progress can commit an attempt, but TTFT starts only when the
// event carries content visible to the client.
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if firstTokenMs == nil && (lineStartsSemanticOutput && trimmedData != "[DONE]" || openAIStreamDataStartsTTFT(trimmedData, eventType, forceFlushFailedEvent, ttftMode)) {
