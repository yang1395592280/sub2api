...
func (w GatewayOpenAIWSSchedulerScoreWeights) BaseWeightSum() float64 {
	return w.Priority + w.Load + w.Queue + w.ErrorRate + w.TTFT + w.Price + w.Reset + w.QuotaHeadroom + w.UpstreamCost
}
...
func (w GatewayOpenAIWSSchedulerScoreWeights) IsValid() bool {
	for _, weight := range []float64{
		w.Priority, w.Load, w.Queue, w.ErrorRate, w.TTFT, w.Price, w.Reset,
...
type GatewayOpenAISchedulerConfig struct {
	...
	PriceBoostMultiplier float64 `mapstructure:"price_boost_multiplier"`
}
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if cfg.Gateway.OpenAIScheduler.PriceBoostSpeedGapMS == 0 {
...
func (c *Config) Validate() error {
	...
	for _, weight := range []float64{
		...
		weights.Price, weights.Reset, weights.QuotaHeadroom, weights.UpstreamCost,
	...
	if c.Gateway.OpenAIScheduler.PriceBoostSpeedGapMS < 0 {
		return fmt.Errorf("gateway.openai_scheduler.price_boost_speed_gap_ms must be non-negative")
	}
