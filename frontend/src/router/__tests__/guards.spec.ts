...
function simulateGuard(
  ...
  if (!requiresAuth) {
    ...
    if (authState.backendModeEnabled && !authState.isAuthenticated) {
      const allowed = ['/login', '/key-usage', '/image-api-docs', '/setup', '/payment/result']
      const callbackPaths = [
        '/auth/callback',
...
describe('路由守卫逻辑', () => {
  ...
  describe('Backend Mode', () => {
    ...
    it('unauthenticated: /image-api-docs is allowed', () => {
      const authState: MockAuthState = {
        isAuthenticated: false,
        isAdmin: false,
        isSimpleMode: false,
      ...
      const redirect = simulateGuard('/image-api-docs', { requiresAuth: false }, authState)
      expect(redirect).toBeNull()
    })
    ...
    it('non-admin authenticated: /image-api-docs is allowed', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: false,
        isSimpleMode: false,
