...
describe('SchedulerRankingTable', () => {
  it('shows effective policy and target versus actual traffic', async () => {
    ...
    expect(wrapper.text()).toContain('综合账号视图')
  ...
  it('shows one physical account as an aggregate across scheduling dimensions', () => {
    const wrapper = mount(SchedulerRankingTable, {
      props: {
        groupName: 'Codex', loading: false, filters: { window: '1h', page: 1, page_size: 20 },
        result: {
          ...
          items: [{
            ...item,
            partition: { group_id: 33, model_family: '', endpoint: '', transport: '' },
            partition_count: 3,
          }],
    ...
    expect(wrapper.text()).toContain('#10 · 聚合 3 个调度维度')
    expect(wrapper.text()).toContain('按真实账号汇总')
