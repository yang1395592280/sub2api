  ...
  <AppLayout>
    <div class="space-y-4">
      ...
      <section v-if="activeTab === 'settings'" class="space-y-4">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
          ...
          <div class="mt-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_auto]">
            <input v-model.trim="grantSearch" type="search" class="input" :placeholder="t('admin.zenxiangLiyu.grantSearchPlaceholder')" @keyup.enter="searchGrantUsers" />
            <button data-testid="zenxiang-search-users" class="btn btn-secondary" :disabled="searchingUsers || !grantSearch" @click="searchGrantUsers">{{ t('common.search') }}</button>
          </div>
      ...
      <section v-else-if="activeTab === 'stats'" class="space-y-4">
        <div class="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-500 dark:border-dark-700 dark:bg-dark-900 dark:text-dark-300">
          {{ t('admin.zenxiangLiyu.statsRangeHint') }}
        </div>
...
async function loadCore() {
  loading.value = true
  try {
    const [settings, prizeList] = await Promise.all([adminAPI.zenxiangLiyu.getSettings(), adminAPI.zenxiangLiyu.listPrizes()])
...
async function loadStats() {
  loading.value = true
  try {
    const [overviewResult, usersResult, prizesResult, grantsResult] = await Promise.all([
