...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.account'), t('admin.accounts.columns.notes'), t('usage.requestedModel'), t('usage.sentUpstreamModel'), t('usage.upstreamResponseModel'), t('usage.upstreamModelMismatch'), t('usage.reasoningEffort'), t('admin.usage.group'),
      ...
      t('usage.rate'), t('usage.accountMultiplier'), t('usage.original'), t('usage.userBilled'), t('usage.accountBilled'), t('usage.profit'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
		log.created_at, log.user?.email || '', log.api_key?.name || '', log.account?.name || '', log.account?.notes || '', log.model,
        ...
        (log.rate_multiplier ?? 1).toPrecision(4), (log.account_rate_multiplier ?? 1).toPrecision(4),
        log.total_cost?.toFixed(6) || '0.000000', log.actual_cost?.toFixed(6) || '0.000000',
        ((log.account_stats_cost ?? log.total_cost) * (log.account_rate_multiplier ?? 1)).toFixed(6),
        (log.actual_cost - ((log.account_stats_cost ?? log.total_cost) * (log.account_rate_multiplier ?? 1))).toFixed(6),
...
const allColumns = computed(() => [
  ...
  { key: 'account_notes', label: t('admin.accounts.columns.notes'), sortable: false },
  ...
  { key: 'rate', label: t('usage.rateAndProfit'), sortable: false },
