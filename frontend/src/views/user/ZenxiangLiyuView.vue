          ...
          <div class="card p-5 sm:p-7">
            <div class="mx-auto flex max-w-md flex-col items-center">
              <div class="relative aspect-square w-full max-w-[19rem]">
                ...
                <div class="zenxiang-wheel" :style="{ background: wheelBackground }">
          ...
          <aside class="card overflow-hidden">
            ...
            <ul v-if="status.prizes.length" class="divide-y divide-gray-100 dark:divide-gray-700">
              <li v-for="prize in status.prizes" :key="prize.id" class="flex items-center justify-between gap-3 px-5 py-3">
                ...
                <span class="shrink-0 text-xs text-gray-500 dark:text-gray-400">{{ formatProbability(prize.probability) }}</span>
              </li>
            </ul>
...
import { playZenxiangLiyu, type ZenxiangLiyuPlayResult } from '@/api/zenxiangLiyu'
...
function formatProbability(probability: number): string {
  return `${Number(probability).toLocaleString(undefined, { maximumFractionDigits: 4 })}%`
}
