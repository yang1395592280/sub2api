  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <ProfileCheckinCard
        ...
        :min-reward="checkinMinReward"
        :max-reward="checkinMaxReward"
...
const checkinMinReward = ref(0)
const checkinMaxReward = ref(0)
...
onMounted(async () => {
  try {
    ...
    checkinEnabled.value = s.checkin_enabled ?? false
    checkinMinReward.value = s.checkin_min_reward ?? 0
    checkinMaxReward.value = s.checkin_max_reward ?? 0
    turnstileEnabled.value = s.turnstile_enabled ?? false
