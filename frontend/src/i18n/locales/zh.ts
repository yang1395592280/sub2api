export default {
  ...
  workbench: {
    title: '网页工作台',
    description: '在浏览器中调试 chat 与 image 工作流',
    conversations: '会话',
    conversationsHint: '按会话组织你的网页工作台请求',
    newConversation: '新建会话',
    untitled: '未命名会话',
    emptyConversation: '还没有消息，先发一条试试。',
    messagesCount: '条消息',
    emptyState: '暂无会话，点击右上角创建一个新的工作台会话。',
    workspaceHint: '通过 /workbench 后端接口统一完成聊天与图片生成。',
    settings: '参数设置',
    settingsHint: '选择 API Key、模型，以及图片生成参数。',
    apiKey: 'API Key',
    selectApiKey: '选择可用 API Key',
    model: '模型',
    selectModel: '选择模型',
    endpoint: '工作台端点',
    modeChat: '聊天',
    ...
    imageWorkflowEdit: '图生图',
    referenceImage: '参考图',
    referenceImageHint: '最多上传 4 张参考图，发送后会调用 images_edits。',
    referenceImageRequired: '请先上传至少一张参考图',
    referenceImageReadFailed: '读取参考图失败',
    removeReferenceImage: '移除参考图',
    inputFidelity: '参考图还原度',
    imageSize: '尺寸',
    imageQuality: '质量',
    imageBackground: '背景',
    imageFormat: '输出格式',
    ...
    imageCount: '生成数量',
    chatPanelTitle: '聊天模式',
    chatPanelBody: '使用右侧选择的 API Key 与模型，消息会通过统一工作台接口发送，不会直接请求上游模型接口。',
    routeOnlyHint: '该页面只通过 `/workbench/...` 相关接口与后端通信。',
    apiKeyRequired: '请选择一个可用的 API Key',
  ...
  zenxiangLiyu: {
    title: '臻享礼遇',
    description: '使用站内余额参与专属礼遇活动',
    currentBalance: '当前站内余额',
    ticketAmount: '本次参与金额',
    remainingPlays: '今日剩余次数',
    configuredRewards: '当前礼遇档位',
    noRewards: '暂无可参与的礼遇档位',
    balanceUnit: '元',
    open: '开启礼遇',
    opening: '开启中',
    refresh: '刷新状态',
    ...
    maintenance: '活动维护中',
    unavailable: '当前暂不可参与',
    playHint: '每次参与将使用 {amount} 元站内余额',
    rewardResult: '本次获得 {amount} 元礼遇额度',
    rewardAmount: '礼遇额度：{amount} 元',
  ...
  admin: {
    zenxiangLiyu: {
      ...
      startDate: '开始日期', endDate: '结束日期', refreshStats: '刷新统计', statsRangeHint: '当前统计接口展示全量数据；后端支持日期筛选后可在此接入区间过滤。', systemRevenue: '系统收入', systemExpense: '系统支出', systemProfit: '系统盈亏', profitRate: '盈亏率', participantsAndPlays: '参与用户 / 次数', userStats: '用户统计', prizeStats: '奖项统计', user: '用户', plays: '参与次数', ticketTotal: '参与额度合计', rewardTotal: '礼遇额度合计', netTotal: '用户净额', hitCount: '命中次数', configuredProbability: '配置概率', actualRate: '实际命中率', probabilityDiff: '偏差', grants: '单账号授权', grantSearchPlaceholder: '输入用户 ID 或邮箱搜索普通用户', grantUserId: '用户 ID', grant: '授权', noGrants: '暂无单账号授权', grantSaved: '授权已保存', grantSaveFailed: '保存授权失败', grantRemoved: '授权已移除', grantRemoveFailed: '移除授权失败', userSearchFailed: '搜索用户失败', statsLoadFailed: '加载统计失败',
      simulatorTitle: '运营模拟', simulatorHint: '模拟仅计算运营结果，不写入真实用户站内余额或真实流水。', simulationPrizeConfig: '模拟奖项配置', userCount: '模拟用户数', playsPerUser: '每用户参与次数', initialBalance: '初始站内余额', targetProfitRate: '目标盈利率', runSimulation: '运行模拟', recommend: '推荐概率', totalPlays: '模拟参与次数', simulationFailed: '运行模拟失败', recommendations: '推荐方案', planSummary: '理论盈亏 {profit}，理论盈利率 {rate}', applyRecommendation: '应用为正式奖项配置', applyToLocalPrizes: '应用到本地奖项配置', recommendationApplied: '推荐方案已应用为正式奖项配置。', recommendationFailed: '生成推荐方案失败', loadFailed: '加载臻享礼遇配置失败',
    },
    workbench: {
      title: '工作台记录',
    ...
    dashboard: {
      ...
      openaiAutoCheapestUsersDialogTitle: 'OpenAI 自动最优惠用户',
      openaiAutoCheapestUsersDialogHint: '当前共有 {count} 个用户选择了 OpenAI 自动选择最优惠分组。',
      openaiAutoCheapestUsersEmpty: '暂无用户选择自动最优惠分组',
      openaiAutoCheapestMaxRate: '最高倍率',
      openaiAutoCheapestMaxRateUnlimited: '不限制',
      users: '用户',
      totalUsers: '用户总数',
      activeUsers: '活跃用户',
      accounts: '账号',
    ...
    groups: {
      ...
      columns: {
        ...
        userNotes: '备注',
        user: '用户',
        currentConcurrency: '当前线程',
        currentRPM: '当前 RPM',
        rpmSource: 'RPM 来源',
        rpmLimits: 'RPM 限制',
        userStatus: '状态'
      },
      capacityUsersTitle: '{name} - 容量使用用户',
      capacityUsersActiveOnly: '仅显示当前有线程或 RPM 的用户',
      noActiveCapacityUsers: '当前没有活跃占用用户',
      viewCapacityUsers: '查看占用用户明细',
      groupRpmLimit: '分组 RPM',
      userRpmLimit: '用户 RPM',
      rpmOverride: '用户覆盖 RPM',
      rpmLimitSources: {
        override: '用户覆盖',
        group: '分组',
        user: '用户',
      ...
      viewMembers: '查看用户',
      membersTitle: '分组用户名单 - {name}',
      membersCount: '{count} 个用户',
      noMembers: '当前分组下暂无用户',
      publicGroupNoFixedMembers: '这是公开标准分组，所有用户都可访问，因此没有固定的用户名单。',
      memberUsageToday: '今天',
      memberUsageYesterday: '昨天',
      memberUsageLoadFailed: '用量加载失败',
      memberUsageNoData: '暂无用量',
      failedToLoadMembers: '加载分组用户失败',
      removeMember: '移除',
      removeMemberConfirm: '确定将该用户移出专属分组，并解绑其在该分组下的 API Key 吗？',
      memberRemoved: '用户已移出分组',
      failedToRemoveMember: '移出分组失败',
    ...
    accounts: {
      ...
      openai: {
        ...
        overbrush: '是否超刷',
        overbrushDesc: '开启后，该 OpenAI API Key 账号遇到 429 会继续调度，连续达到全局阈值后才进入限流中。',
      ...
      channelPrice: '渠道价格',
      channelPriceHint: '上游真实价格，越低在同层级、速度接近时越优先',
      channelPriceInvalid: '渠道价格必须大于 0',
      expiresAt: '过期时间',
      ...
      testResultSuccess: '已通过',
      testResultFailed: '未通过',
      selectTestModel: '选择测试模型',
      bulkTestSummaryTitle: '批量测试结果',
      bulkTestProgress: '当前进度：已完成 {current} / {total} 个账号',
      bulkTestHint: '按设定并发数对已选账号执行当前连接测试。',
    ...
    settings: {
      ...
      site: {
        ...
        joinGroup: {
          enabled: '启用加入群聊入口',
          enabledHint: '关闭时首页和顶部导航都会直接隐藏“加入群聊”按钮。',
          url: '群聊链接',
          urlPlaceholder: 'https://qm.qq.com/q/...',
          urlHint: '仅支持 http/https；开启入口但留空链接时，前台仍会隐藏按钮。',
          image: '弹窗图片',
          imagePlaceholder: '上传群二维码或群介绍图片',
          imageHint: '配置后点击“加入群聊”会优先弹出图片预览；若同时填写群聊链接，弹窗内仍可继续跳转。'
        },
        siteLogo: '站点Logo',
        uploadImage: '上传图片',
        remove: '移除',
