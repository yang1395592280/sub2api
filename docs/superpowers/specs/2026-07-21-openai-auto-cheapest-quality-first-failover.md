...
7. 自动最优惠 API Key 在同一用户价格分组内，先完成健康、能力、价格保护和运行时过滤，再按账号 `channel_price` 升序分层尝试。
8. 同一 `channel_price` 内仍复用 Balanced 的健康、负载和公平性排序；低价层无法立即取得并发槽位时，才尝试下一渠道价格层。
9. 不可迁移的 `previous_response_id` 继续强粘连原账号；普通会话粘性不得将新请求长期固定在更贵账号上。
...
如果统一健康读取失败、自动调度未启用、模式为 Legacy 或 Shadow 开启，则质量过滤不生效；自动最优惠 API Key 的同组 `channel_price` 分层仍生效，同价层内保持当前真实选择顺序。
...
同组渠道价格分层属于“自动最优惠” API Key 的成本语义，不受 Shadow 开关影响；Shadow 仅决定统一健康资格和 Balanced 质量顺序是否影响真实流量。

## 10. 代码改动

| 文件 | 改动 |
| --- | --- |
| `openai_auto_cheapest_group_context.go` | 增加质量扫描、同组渠道价格优先和首字超时后的禁止兜底状态 |
| `openai_gateway_scheduling.go` | 自动最优惠执行质量扫描、组内渠道价格分层和可用性兜底 |
| `openai_account_scheduler.go` | 请求级仅达标账号过滤、同组渠道价格排序和普通粘性价格逃逸，保持固定分组行为不变 |
...
7. 健康存储读取失败时回退原价格优先。
8. 固定分组 API Key 不受影响。
9. 同组存在观察中 `0.03`、较慢但达标的 `0.04` 和较快且达标的 `0.05` 账号时，选择 `0.04`。
10. 同组低价达标账号已失败、被排除或无可用并发槽位时，选择下一渠道价格层。
11. 普通会话粘在更贵账号时逃逸到低价达标账号，不可迁移的 `previous_response_id` 仍命中原账号。
