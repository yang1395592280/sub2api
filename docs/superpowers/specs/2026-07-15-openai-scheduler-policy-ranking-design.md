...
- 模型族：可选，用于限制参与综合排名的调度维度。
- Endpoint：默认请求量最高的 endpoint。
- Transport：默认请求量最高的 transport。
- 状态：全部、合格、低置信度、延迟尾部、熔断、硬过滤。
- 流量类型：全部、新会话、粘性、回退。

调度策略仍按 `account_id + model_family + endpoint + transport` 分区计算，但排名页面始终按真实 `account_id` 聚合，同一物理账号只能出现一行。模型、端点和传输筛选器只限制参与聚合的分区，不改变账号唯一性。

跨分区综合口径：

- 目标份额按各分区在统计窗口内的真实请求占比加权；窗口完全无请求时，各分区等权。
- 实际份额按账号窗口总请求数除以分组窗口总请求数计算。
- 综合效用和分项分数按分区请求占比加权，无流量账号仍保留为低置信度或回退候选。
- 性能和稳定性指标按窗口请求量或分区权重聚合，成本按账号窗口成本求和。
- API 返回 `partition_count`，前端显示“聚合 N 个调度维度”，不得将不同模型维度摊平成重复账号行。
...
| 调度维度 | 当前账号聚合的模型、Endpoint、Transport 分区数量 |
